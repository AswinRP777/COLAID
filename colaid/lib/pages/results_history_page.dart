...
class _ResultsHistoryPageState extends State<ResultsHistoryPage> {
  ...
  Future<void> _deleteSelected() async {
    final shouldDelete = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Are you sure you want to delete ${_selectedDates.length} result(s)?"),
        actions: [
  ...
  Widget build(BuildContext context) {
    final results = UserService().getTestResults(); // This might need setState to refresh if not reactive
    ...
    return Scaffold(
      appBar: AppBar(
        leading: _isSelectionMode 
          ...
          : null,
        title: Text(_isSelectionMode ? "${_selectedDates.length} Selected" : "Test History"),
        actions: [
          if (_isSelectionMode)
            IconButton(
              ...
              itemBuilder: (context, index) {
                ...
                try {
                  if (result['plates'] != null) {
                    ...
                    plates = (result['plates'] as List).map((e) => Map<String, dynamic>.from(e as Map)).toList();
                  }
                  
                  if (result['userAnswers'] != null) {
                    final Map<dynamic, dynamic> rawAnswers = result['userAnswers'] as Map;
                    userAnswers = rawAnswers.map((k, v) => MapEntry(int.parse(k.toString()), v.toString()));
                  }
                } catch (e) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    leading: _isSelectionMode
                      ...
                      : CircleAvatar(
                          backgroundColor: type == "Normal Vision" ? Colors.green.shade100 : Colors.orange.shade100,
                          child: Icon(
                            type == "Normal Vision" ? Icons.check : Icons.warning_amber,
                            color: type == "Normal Vision" ? Colors.green : Colors.deepOrange,
                          ),
                        ),
                    ...
                    subtitle: Column(
                      ...
                      children: [
                        ...
                        Text(dateStr),
                        const SizedBox(height: 4),
                        Text("Incorrect Answers: $incorrect", style: const TextStyle(fontWeight: FontWeight.w500)),
                        if (platesCount > 0)
                           Text("Details available ($platesCount plates)", style: const TextStyle(color: Colors.blue, fontSize: 12)),
                      ],
                    ),
                    onLongPress: () {
                      setState(() {
                    ...
                    onTap: _isSelectionMode 
                      ? () => _toggleSelection(dateKey)
                      : (plates != null && userAnswers != null && plates.isNotEmpty) ? () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text("Detailed results not available for this entry.")),
                        );
                      }),
                  ),
                );
